...
export function OnboardEmployeeView() {
  ...
  async function submit() {
    ...
    const idx = last ? Number(String(last.id).split('-').pop()?.replace(/[^0-9]/g, '')) || 1000 : 1000
    const newId = draft.id && String(draft.id).trim() ? String(draft.id).trim() : `EMP-${idx + 1}`
    const newEmp: Employee = {
      id: newId,
      name: String(draft.name),
      avatar: draft.profilePhoto?.data || draft.avatar || undefined,
      mobile: String(draft.phone || ''),
      email: String(officialEmail || draft.email || ''),
      designation: String(draft.designation || ''),
      team: String(draft.department || ''),
      status: draft.status || 'Active',
      score: Number(draft.score || 0),
      rank: employees.length + 1,
      tasksClosed: Number(draft.tasksClosed || 0),
      joiningDate: String(draft.joiningDate || ''),
      address: String(draft.address || ''),
      incentiveEarned: Number(draft.incentiveEarned || 0),
      assignedTasks: Number(draft.assignedTasks || 0),
      completedTasks: Number(draft.completedTasks || 0),
      pendingTasks: Number(draft.pendingTasks || 0),
      attendance: [],
    }
