      ...
      <div className="mt-6 px-3 md:px-6">
        <div className="flex items-center justify-end mb-6 flex-col md:flex-row md:items-center">
          <div className="flex items-center gap-3 mr-auto w-full md:w-auto">
            ...
            <Button className="w-full md:w-auto" variant="outline" size="sm" onClick={() => { const rows = (tasks || []).map(t => ({ id: t.id, name: t.name, client: t.client, status: t.status })); downloadCsv(rows, 'tasks.csv'); toast({ type: 'success', message: 'Exported tasks' }) }}>Export</Button>
          </div>
