...
export default function EditEmployeePage() {
  ...
  useEffect(() => {
    ...
    if (!found) setError('Employee not found')
      ...
      const defaults: FormValues = {
        name: found.name,
        id: found.id as any,
        officialEmailLocal: (found.email || '').split('@')[0],
        profilePhoto: (found as any).profilePhoto,
        kycUploads: (found as any).kycUploads || {},
      }
      reset(defaults)
    }
  }, [id, reset])
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      if (idx === -1) throw new Error('Employee not found during save')
      const merged = { ...employees[idx], ...(values as any) }
      if (values.officialEmailLocal) merged.email = `${values.officialEmailLocal}@complianceos.com`
      employees[idx] = merged as Employee
            ...
            <Card>
              ...
              <div className="px-4 pb-4 flex flex-col items-center">
                <div className="w-36 h-36 rounded-full bg-gray-100 flex items-center justify-center relative">
                  ...
                  ) : <div className="text-3xl font-semibold">{employee.name.split(' ').map(n => n[0]).slice(0, 2).join('')}</div>}
                </div>
