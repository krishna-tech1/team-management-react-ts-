...
export default function AdminEmployees() {
  ...
  const [items, setItems] = useState<Employee[]>(() => employees.slice())
  ...
  const [kpi, setKpi] = useState(() => computeKpi(employees.slice()))
  ...
  useEffect(() => {
    function onChanged() {
      // bump version so filtering effect re-runs and reapplies current filters/search/sort
  ...
  useEffect(() => {
    ...
    setItems(filtered)
  }, [query, statusFilter, sortKey, sortDir, advancedFilters, dataVersion])
  ...
  useEffect(() => {
    ...
    // recompute KPIs using computeKpi to avoid duplicated logic
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-4">
        <div className="lg:col-span-3">
          <Card className="mt-2">
                ...
                <div className="relative">
                  ...
                  {showStatusDropdown && (
                    <div className="absolute mt-2 w-48 rounded-md border bg-white shadow p-2 z-40">
                      ...
                      {['Active','Inactive','On Leave', ...Array.from(new Set(employees.map((e) => e.status)))].filter(Boolean).map((s) => (
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
                  <div>
                    ...
                    <select value={advancedFilters.department || ''} onChange={(e) => setAdvancedFilters((a: any) => ({ ...a, department: e.target.value || undefined }))} className="mt-1 h-10 w-full rounded-lg border border-line bg-surface px-3 text-sm text-ink">
                      ...
                      {[...new Set(employees.map(e => e.team))].map(d => <option key={d} value={d}>{d}</option>)}
                    </select>
                  </div>
                  <div>
                    ...
                    <select value={advancedFilters.designation || ''} onChange={(e) => setAdvancedFilters((a: any) => ({ ...a, designation: e.target.value || undefined }))} className="mt-1 h-10 w-full rounded-lg border border-line bg-surface px-3 text-sm text-ink">
                      <option value="">Any</option>
                      {[...new Set(employees.map(e => e.designation))].map(d => <option key={d} value={d}>{d}</option>)}
                    </select>
                  </div>
                  <div>
                    ...
                    <select value={advancedFilters.status || ''} onChange={(e) => setAdvancedFilters((a: any) => ({ ...a, status: e.target.value || undefined }))} className="mt-1 h-10 w-full rounded-lg border border-line bg-surface px-3 text-sm text-ink">
                      <option value="">Any</option>
                      {['Active','Inactive','On Leave', ...Array.from(new Set(employees.map((e) => e.status)))].filter(Boolean).map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>
                  <div>
            ...
            {popoverOpenFor && (
              (() => {
                const emp = employees.find((e) => e.id === popoverOpenFor)
                if (!emp) return null
                return (
                  <EmployeeQuickPanel
                    employee={emp}
                    ...
                    onDeactivate={() => {
                      ...
                      if (idx === -1) return
                      const newStatus = employees[idx].status === 'Inactive' ? 'Active' : 'Inactive'
                      if (confirm(`${newStatus === 'Inactive' ? 'Deactivate' : 'Activate'} ${emp.name}?`)) {
                        employees[idx] = { ...employees[idx], status: newStatus }
                        // rely on the global event listener to refresh local items and KPIs
